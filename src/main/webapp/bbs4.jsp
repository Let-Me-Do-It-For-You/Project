...
<%@ page import="bbs.BbsDTO4"%>
<%@ page import="java.util.ArrayList"%>
<%@ page import="java.net.URLEncoder"%>
<!doctype html>
<html>
<head>
<title>물건 운반 대행</title>
<meta charset="utf-8">
...
<body>
	...
	<nav class="navbar navbar-expand-sm bg-info navbar-dark">
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
				...
				<li class="active"><a class="nav-link" href="bbs4.jsp">물건 운반 대행</a></li>
	...
	<div class="container">
		<form method="get" action="bbs4.jsp" class="form-inline mt-3">
	...
	out.println("selected");%>>금액순</option>
		...
		<%
		ArrayList<BbsDTO4> bbsList = new ArrayList<BbsDTO4>();
		bbsList = new BbsDAO4().getList(hjsy, searchType, search, pageNumber);
		if (bbsList != null)
			for (int i = 0; i < bbsList.size(); i++) {
				if (i == 5)
			break;
				BbsDTO4 bbs = bbsList.get(i);
		%>
			...
			<div class="card-body">
				...
				<div class="row">
					...
					<div class="col-3 text-right">
						<a onclick="return confirm('추천하시겠습니까?')" href="./likeAction4.jsp?bbsID=<%=bbs.getBbsID()%>">추천</a> <a onclick="return confirm('삭제하시겠습니까?')" href="./deleteAction4.jsp?bbsID=<%=bbs.getBbsID()%>">삭제</a>
					</div>
				</div>
			</div>
		</div>
 ...
 %> <a class="page-link" href="bbs4.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber - 1%>">이전</a> <%
 }
 %>
		</li>
 ...
 %> <a class="page-link" href="bbs4.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber + 1%>">다음</a> <%
 }
 %>
		</li>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="modal">물건 운반 대행</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form action="bbsRegisterAction4.jsp" method="post">
						<div class="form-row">
