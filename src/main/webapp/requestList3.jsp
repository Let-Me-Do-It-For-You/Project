...
<body>
	...
	<div class="container">
		...
		ArrayList<BbsDTO1> bbsList = bbsDAO.getProgressList(userID, hjsy, searchType, search, pageNumber);
		if (bbsList != null)
			...
			<div class="card-body">
				...
				<div class="row">
					<div class="col-9 text-left">
						매장 위치 : <%String restaurantName = bbs.getRestaurantName(); if (restaurantName != null) {%>
						<a style="color: green; cursor: pointer;" onClick="goPopup2()"><%= restaurantName %></a>
						<%} else {%> 위치 없음 <%}%>
					</div>
					<div class="col-12 text-right">
						<form id="approveForm<%=i%>" action="approveAction.jsp" method="get">
							<input type="hidden" name="bbsID" value="<%=bbsList.get(i).getBbsID()%>">
							<%
							if (!bbs.getLikeClose()) {
							%>
							<a style="color: red;">동참 진행중</a>
							<%
							} else {
								if (bbs.getRiderProgress() == 2) {
									%>
									<button type="submit" class="btn btn-secondary" name="approve" value="p">음식 픽업</button>
									<%
								}
								else if (bbs.getRiderProgress() == 3) {
									%>
									<button type="submit" class="btn btn-warning" name="approve" value="d">배달 진행중</button>
									<%
								}
								else if (bbs.getRiderProgress() == 4) {
									%>
									<button type="submit" class="btn btn-primary" name="approve" value="c">배달 완료</button>
									<%
								}
								else if (bbs.getRiderProgress() == 5) {
									%>
									<button type="submit" class="btn btn-primary" name="approve" value="c">배달 종료</button>
									<%
								}
							}
							%>
		...
		<ul class="pagination justify-content-center mt-3">
			<li class="page-item">
				...
				%> <a class="page-link" href="requestList3.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber - 1%>">이전</a>
				<%
				}
				%>
			</li>
			...
			<li class="page-item">
				...
				%> <a class="page-link" href="requestList3.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber + 1%>">다음</a>
				<%
				}
				%>
			</li>
		...
		<script>
			...
			function goPopup(restaurantAddress, restaurantName) {
		        var url = "kakaoMap2.jsp?restaurantAddress=" + restaurantAddress + "&restaurantName=" + restaurantName;
		        var pop = window.open(url, "pop", "width=500,height=400,scrollbars=yes,resizable=yes");
		    }
