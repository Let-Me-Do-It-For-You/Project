...
<body>
	...
	<div class="container">
		...
		ArrayList<BbsDTO1> bbsList = bbsDAO.getRiderList(userID, hjsy, searchType, search, pageNumber);
		if (bbsList != null)
				...
				<div class="row">
					...
					<div class="col-3 text-right">
						<form id="approveForm<%=i%>" action="approveAction.jsp" method="get">
							...
							<button type="submit" class="btn btn-danger" onclick="return confirm('거절하시겠습니까?')" name="approve" value="n">거절</button>
						</form>
					</div>
				</div>
			</div>
		...
		<ul class="pagination justify-content-center mt-3">
			<li class="page-item">
				...
				%> <a class="page-link" href="requestList.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber - 1%>">이전</a>
				<%
				}
				%>
			</li>
			...
			<li class="page-item">
				...
				%> <a class="page-link" href="requestList.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber + 1%>">다음</a>
				<%
				}
				%>
			</li>
