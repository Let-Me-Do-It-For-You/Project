...
<body>
	...
	<nav class="navbar navbar-expand-sm bg-info navbar-dark">
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
				...
				<li class="active"><a class="nav-link" href="bbs3.jsp">프린트 대행</a></li>
	...
	<div class="container">
		<form method="get" action="bbs3.jsp" class="form-inline mt-3">
	...
	out.println("selected");%>>금액순</option>
		...
		<%
		ArrayList<BbsDTO3> bbsList = new ArrayList<BbsDTO3>();
		bbsList = new BbsDAO3().getList(hjsy, searchType, search, pageNumber);
		if (bbsList != null)
			for (int i = 0; i < bbsList.size(); i++) {
				if (i == 5)
			break;
				BbsDTO3 bbs = bbsList.get(i);
		%>
			...
			<div class="card-body">
				...
				<div class="row">
					...
					<div class="col-3 text-right">
						<a onclick="return confirm('추천하시겠습니까?')" href="./likeAction3.jsp?bbsID=<%=bbs.getBbsID()%>">추천</a> <a onclick="return confirm('삭제하시겠습니까?')" href="./deleteAction3.jsp?bbsID=<%=bbs.getBbsID()%>">삭제</a>
					</div>
				</div>
			</div>
		</div>
 ...
 %> <a class="page-link" href="bbs3.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber - 1%>">이전</a> <%
 }
 %>
		</li>
 ...
 %> <a class="page-link" href="bbs3.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber + 1%>">다음</a> <%
 }
 %>
		</li>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="modal">프린트 대행 글작성</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form action="bbsRegisterAction3.jsp" method="post">
						<div class="form-row">
