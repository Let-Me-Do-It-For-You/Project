...
<body>
			...
			<ul class="navbar-nav">
				...
				<li class="active dropdown"><a class="nav-link dropdown-toggle" id="dropdown" data-toggle="dropdown"><%=userN.getUserID()%>의 마이페이지</a>
					<div class="dropdown-menu" aria-labelledby="dropdown">
						...
						if (rider.equals("1")) {
						%>
						<a class="dropdown-item" href="requestList.jsp">승인 요청</a> <a class="dropdown-item" href="requestList2.jsp">요청 대기 목록</a> <a class="dropdown-item" href="requestList3.jsp">진행 사항</a>
						<%
						}
						%>
	...
	<div class="container">
		...
		ArrayList<BbsDTO1> bbsList = bbsDAO.getRequestList(hjsy, searchType, search, pageNumber);
		if (bbsList != null)
			...
			<div class="card-body">
				...
				<div class="row">
					<div class="col-9 text-left">
						...
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (라이더 모집중)] </span>
						<%
						} else if (bbs.getRiderProgress() == 1) {
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (요청 대기중)] </span>
						<%
						} else if (bbs.getRiderProgress() == 2) {
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (요청 완료)] </span>
						<%
						} else if (bbs.getRiderProgress() == 3) {
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (음식 픽업)] </span>
						<%
						} else if (bbs.getRiderProgress() == 4) {
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (배달 진행중)] </span>
						<%
						} else if (bbs.getRiderProgress() == 5) {
						%><span style="color: purple;"> [라이더: <%=bbs.getRider()%> (배달 완료)] </span>
						<%
						}
						%>						
					</div>
					<div class="col-3 text-right">
						<form id="approveForm<%=i%>" action="approveAction.jsp" method="get">
							...
							<button type="submit" class="btn btn-success" onclick="return confirm('승인하시겠습니까?')" name="approve" value="y">수락</button>
						</form>
					</div>
				</div>
			</div>
 ...
 %> <a class="page-link" href="requestList2.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber - 1%>">이전</a> <%
 }
 %>
			</li>
 ...
 %> <a class="page-link" href="requestList2.jsp?hjsy=<%=URLEncoder.encode(hjsy, "UTF-8")%>&searchType=<%=URLEncoder.encode(searchType, "UTF-8")%>&search=<%=URLEncoder.encode(search, "UTF-8")%>&pageNumber=<%=pageNumber + 1%>">다음</a> <%
 }
 %>
			</li>
